                ...
                <div id="left" class="col-md-8 border-right">  
                    <div class="form-group col-md-12 no-padding">
                        ...
                        <div class="input-group">
							...
							<select name="cboHangHoa" id="cboHangHoa" class="form-control select2" style="width: 100%;">
								<option value="">&nbsp;</option>
								<?php 
								$sanpham_list = $this->sanpham_model->lay_danh_sach_san_pham_autocomplete("");
								foreach($sanpham_list as $sanpham)
								{
								?>
								<option value="<?php echo $sanpham["sp_id"];?>"><?php echo $sanpham["sp_ten"];?></option>
								<?php
								}
								?>
							</select>
						...
						<div class="input-group">
							...
							<input type="text" id="txtHangHoa" name="txtHangHoa" class="form-control" placeholder="Tìm kiếm hàng hóa (F3)"/>

                        </div>
                    </div>
                    <div id="donhangchitiet" class="form-group col-md-12 no-padding table-responsive">
...
$(function () {
	$("#txtHangHoa").autocomplete({
		minLength: 1,
		source: '/admin/Ajaxdonhang/cms_autocomplete_products/',
		focus: function (event, ui) {
			$("#txtHangHoa").val(ui.item.sp_id);
			return false;
		},
		select: function (event, ui) {
			//alert(ui.item.sp_id);
			//cms_select_product_sell(ui.item.sp_id);
			ThemDonHangChiTiet('<?php echo $donhang['dh_id'];?>',ui.item.sp_id, ui.item.sp_gia_ban,1);
			$("#txtHangHoa").val('');
			return false;
		}
	}).keyup(function (e) {
	...
	.autocomplete("instance")._renderItem = function (ul, item) {
		/*
		return $("<li>").append("<a onclick=\"ThemHoaDonChiTiet('<?php echo $donhang['dh_id'];?>','"+item.sp_id+"')\">")
						.append("<img class='ui-menu-item-image' src='/uploads/sanpham/" + item.sp_hinh + "' align='left'/>")
						.append("<p class='ui-menu-item-text'>" + item.sp_ten + "</p>")
						.append("<p>" + formatCurrency(item.sp_gia_ban) + "</p>")
						.append("</a>")
						.appendTo(ul);
		*/
		return $("<li>").append("<a>")
...
function LuuDong()
{
	...
	tienhang = tienhang.replace(re,"");
	//alert(tienhang);
	var phivanchuyen = $('#txtPhiVanChuyen').val();
	phivanchuyen = phivanchuyen.replace(re,"");
	
	var tongcong = $('#txtTongCong').val();
	tongcong = tongcong.replace(re,"");
	//alert(tongcong);
	var thanhtoan = $('#txtThanhToan').val();
	thanhtoan = thanhtoan.replace(re,"");

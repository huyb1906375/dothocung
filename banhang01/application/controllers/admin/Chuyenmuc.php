...
class Chuyenmuc extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->data['com']='chuyenmuc';
	}
	public function index()
	{
		$this->data['template']='admin/chuyenmuc/index';
	...
	public function add()
	{
		...
		{			
			...
			$mydata= array(
				...
				'cm_link' => "/chuyen-muc/".$this->alias->str_alias($this->input->post('txtTen')).".html",
			...
			if($this->chuyenmuc_model->them_chuyen_muc($mydata))
			{
				...
				$this->session->set_userdata('cm_noi_bat',$this->input->post('cboNoiBat'));
				$this->session->set_userdata('cm_menu',$this->input->post('cboMenu'));
				redirect('admin/chuyenmuc/add','refresh');
			}
			else
			{
				$this->session->set_flashdata('error', 'Không thêm được chuyên mục này!');
				$this->data['template']='admin/chuyenmuc/add';
				$this->data['title']='Thêm chuyên mục - SutekCMS';
				$this->load->view('admin/index', $this->data);
	...
	public function edit()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			if($this->chuyenmuc_model->sua_chuyen_muc($mydata, $id))
			{
				...
				$this->session->set_flashdata('success', 'Sửa chuyên mục thành công');	
				$this->session->set_userdata('cm_id_parent',$this->input->post('cboChuyenMucCha'));
				$this->session->set_userdata('cm_trang_thai',$this->input->post('cboTrangThai'));
				$this->session->set_userdata('cm_noi_bat',$this->input->post('cboNoiBat'));
				$this->session->set_userdata('cm_menu',$this->input->post('cboMenu'));
			...
			{
				$this->session->set_flashdata('error', 'Không sửa được chuyên mục này!');
				$this->data['template']='admin/chuyenmuc/edit';
				$this->data['title']='Sửa chuyên mục - SutekCMS';
				$this->load->view('admin/index', $this->data);
